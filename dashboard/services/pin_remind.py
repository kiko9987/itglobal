...
# 세금계산서(지출) 매칭 요청 — "MM/DD G/R/N 금액 거래처 …" (샛별 표준 양식).
# 2026-09-10: 금액 단위 '원'은 오타가 잦아(운·왼 등 IME 슬립) 기준에서 제외 — **구조로 판정**.
#   MM/DD + G/R/N + 돈 형태 금액(콤마 3자리 그룹 or 4자리 이상) + 뒤에 내용(단위/거래처).
#   금액을 콤마그룹·4자리+로 한정해 '09/10 G 12 회의' 같은 소액/비금액 오탐 방지.
#   단위 글자를 안 보므로 원 오타(운/왼/웜…)·단위 누락 모두 인식. is_deposit(계좌번호)는 별도.
# 이미지 유무 무관. '계약서 진행'·'계산서 미발행건' 등은 MM/DD+G/R/N 구조 아니라 미매치.
_INVOICE_RE = re.compile(
    r'^\s*\d{1,2}/\d{1,2}\s+[GRNgrn]\s+(?:\d{1,3}(?:,\d{3})+|\d{4,})\s*\S')
