...
_ROAD_BEONJI_SPLIT_RE = re.compile(r'([가-힣]{2,}\d*[가-힣]*로)(\d+(?:-\d+)?)(?![가-힣0-9번])')  # 동탄반석로172 → 동탄반석로 172, 향기2로21 → 향기2로 21 (도로명 내부 숫자 허용, L-03828)
_GIL_BEONJI_SPLIT_RE = re.compile(r'(\d+번?길)(\d+(?:-\d+)?)(?![가-힣0-9])')     # 13길4 → 13길 4
# 도로명/길 + 번지 + 건물명 완전 붙여쓰기 (2026-08-26 L-03806): '가람로124봉명타워'
#   → '가람로 124 봉명타워'. 위 BEONJI/GIL_SPLIT 은 번지 뒤가 한글이면 미분할(로124번길·
#   13길4 도로연속 보호)이라 건물 글루를 못 폄. 도로연속(번?로/길)·번지단위(동/호/층/가)
#   아닌 건물명(2자+ 한글)이 붙었을 때만 양쪽 공백 삽입 → verify 가능해짐(시 미제거·글루 해소).
_ROAD_BEONJI_BLDG_SPLIT_RE = re.compile(
    r'([가-힣]{2,}\d*[가-힣]*(?:로|길))(\d+(?:-\d+)?)(?!번?[로길])(?![동호층가])([가-힣]{2,})')  # 가람로124봉명타워 → 가람로 124 봉명타워 (도로명 내부 숫자 허용, L-03828)
...
def _normalize_road_spacing(s: str) -> str:
    ...
    # 완전 붙여쓰기(로+번지+건물)를 BEONJI_SPLIT 보다 먼저 — 안 그러면 BEONJI_SPLIT 가
    #   백트래킹으로 '가람로 124-5봉명타워'처럼 번지만 떼고 건물을 남김 (L-03806).
    s = _ROAD_BEONJI_BLDG_SPLIT_RE.sub(r'\1 \2 \3', s)  # 가람로124봉명타워 → 가람로 124 봉명타워
