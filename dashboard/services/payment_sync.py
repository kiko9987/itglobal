...
# 기업 452-039388-01-011 (글로벌 G), 하나 255-910014-31304 (글로벌그룹 R), 농협 352-****-1682-33
...
# ITG 카드 가맹점 승인번호 + 실제 정산 적요 숫자열 (대표·경영지원 제공 2026-09-01).
# 카드사가 통장에 찍는 카드매출 정산 '입금자(적요)'는 형태가 제각각이다:
#   접두 브랜드 '현108017094' / 접미 브랜드 '720364972BC','745389850B' / 'BC-720364972' /
#   앞자리 절삭 '롯데59366153'(9959366153)·'롯데26648732'(9926648732) /
#   뒷자리만 '삼성카드962'(178390962) / 앞 6~8자리 'NH15415440'(154154402)·'SHC0117935' 등.
# 그래서 가맹점 번호(풀) + 실제 적요 숫자열을 함께 저장하고 양방향 부분일치로 판별한다.
# 숫자가 너무 짧은 '삼성카드962'류는 브랜드+'카드' 단어로 잡는다.
_ITG_CARD_SIGNATURES = frozenset(s for s in (
    # 가맹점 번호(풀) — 아이티글로벌그룹(R·하나) / 아이티글로벌(G·기업)
    ...
    # 실제 정산 적요 숫자열(절삭·변형형·신규 가맹점)
    '0117935', '59366153', '90242344', '90718642', '15415440',
    '26648732', '12194399', '14322878',
) if len(s) >= 6)

# 브랜드+'카드' 단어 (예: '삼성카드962') — 숫자가 짧아 승인번호로 못 잡는 적요용.
_CARD_BRAND_WORD_RE = re.compile(
    r'(?:비씨|BC|삼성|현대|롯데|신한|하나|국민|KB|NH|SH|SHC|우리|씨티|카카오)\s*카드'
)


def _is_itg_card_deposit(text: str) -> bool:
    """입금자(적요)가 ITG 카드매출 정산이면 True — Y열·매니저 입력 무관 확정 신호.

    ① 브랜드+'카드' 단어('삼성카드962') 또는
    ② 입금자 숫자열이 가맹점 승인번호/실제 적요 숫자열과 6자리 이상 부분일치(양방향).
    은행 이체 입금자는 상호명(긴 숫자열 거의 없음)이라 오탐 사실상 없음.
    """
    if not text:
        return False
    if _CARD_BRAND_WORD_RE.search(text):
        return True
...
def _correct_payment_card(slack, channel: str, corr: Dict,
    ...
    if old:  # 정정 전 스냅샷 → 스레드 (무슨 내용인지 명확히 안내, 2026-07-30)
        ...
        _snap = (
            f'🔄 *수금 카드 갱신* ({now})\n'
            f'입금 내역이 변경되어 원본 메세지를 최신 내용으로 업데이트했습니다.\n'
            f'아래는 갱신 전 옛 카드입니다 (기록 보존용).\n'
            f'최신 금액·입금자·내용은 원본 메세지를 확인해 주세요.\n'
            f'{_SEP}\n{old.strip()}'
        )
        try:
...
def _sync_payments_locked(result, sheet_id, sheet_name, channel, bot_token):
    ...
    for c in changed_rows[:MAX_PER_TICK]:
        ...
        try:
            ...
            # 새 상태 저장 — 단계 phash 는 값>0 일 때만 확정 (값 없이 노트만 보이는
            # 시점차 사이클에 phash 가 굳어 카드 유실되는 것 방지, 위 게이트와 동일 기준)
