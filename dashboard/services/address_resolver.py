...
def _post_normalize_display(addr: str) -> str:
    ...
    addr = re.sub(
        ...
        '', addr)
    # 끝 괄호 단지명 → 괄호 벗기고 상가/동 앞(도로+번지 뒤)으로 (2026-09-09 L-03962):
    #   신도시 상가주소 '도로 번지 상가 N동 N층 N호 (XX도시 N단지)' → '도로 번지 XX도시
    #   N단지 상가 N동 …'. **단지 접미**(단지/아파트/마을/타운)로 끝나는 끝 괄호 + 도로+번지
    #   존재 시만 → 노트('(비밀번호 …)')·법정동 괄호는 접미 달라 미대상. 단지명 verbatim
    #   (내부 공백 유지 — 공식 붙임형은 bdNm 필요라 보류). _INST_PAREN(단일토큰 벗기기)이
    #   공백없는 단지명을 in-place 벗기기 전에 실행해 재배치까지 하도록 순서 앞.
    _m_dj = re.match(
        r'^(.*?[가-힣]+(?:로|길)\s*\d+(?:-\d+)?)\s+(.*?)\s*'
        r'\(([^()]*(?:단지|아파트|마을|타운))\)\s*$',
        addr)
    if _m_dj:
        _pre, _tail, _dj = (_m_dj.group(1), _m_dj.group(2).strip(),
                            _m_dj.group(3).strip())
        addr = f'{_pre} {_dj} {_tail}'.strip() if _tail else f'{_pre} {_dj}'
    ...
    # 끝 말끝(문장 종결) 제거 (2026-09-15 L-04024): 고객이 '…폭스라이브 입니다'처럼 말끝을
    #   붙이면 주소 아님 → 제거. 공백형·붙임형 모두. '입니다'로 끝나는 상호는 없음(동사 어미).
    addr = re.sub(r'\s*(?:입니다|입니당|이에요|이예요|예요|이요)\s*$', '', addr)
...
def _enrich_verified_address(
    ...
    # 건물 접미로 끝나는 이름 앞 (주)/㈜ 제거 (2026-09-08 L-03959): 건물은 주식회사가
    #   아니므로 '(주)뉴빛빌딩'→'뉴빛빌딩'. 건물 접미(빌딩/타워/…) 로 끝날 때만 → 접미
    #   없는 상호('(주)삼인')의 (주)는 회사 표기라 보존(L-03716). 어순 복원 전에 실행해
    #   건물명이 한글 시작이 되도록(아래 _m_ord 매치 가능).
    verified_addr = re.sub(
        r'(?:\(주\)|㈜)\s*([가-힣A-Za-z0-9]{1,20}?(?:빌딩|타워|프라자|플라자|스퀘어|'
        r'하우스|맨션|오피스텔|파크|타운|시티))(?![가-힣])',
        r'\1', verified_addr)
    ...
    # 건물명 앞 (주)/㈜·괄호 접두 허용 (2026-09-08 L-03959: '(주)삼인' 등 접미 없어
    #   위 strip 미대상인 (주) 상호도 어순 복원되게).
    _m_ord = re.search(
        r'(\d+(?:-\d+)?)\s+([A-Za-z]?\d+(?:~\d+)?(?:층|호|호실|관))\s+'
        r'((?:\(주\)|㈜)?[가-힣][가-힣A-Za-z0-9]*)$',
        verified_addr,
    )
    if _m_ord:
