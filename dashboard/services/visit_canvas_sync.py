...
def _render_item(lead: Dict, initial_map: Dict[str, str]) -> str:
    ...
    # 공사 확정 건 구분 마커 (2026-07-27) — 계약 후 현장 핸들링 방문. 방문 예약과 구별.
    _confirmed_marker = ''
    if str(lead.get('상태') or '').strip() == '공사 확정':
    ...
    if is_online:
        ...
        if _platform == '당근':
            vd = f'(당) {vd}'
        return f'{_confirmed_marker}{_self_visit_marker}{vd} / {phone} / {address} / {inquiry}'
    ...
    return f'{_confirmed_marker}{_self_visit_marker}({ini}) {vd} / {phone} / {address} / {inquiry}'
...
def _fetch_visit_leads() -> List[Dict]:
    ...
    for _, row in df.iterrows():
        ...
        # 2026-07-27: 공사 확정 건도 포함 — 계약 후 현장 핸들링·기타 방문 잡기 위함.
        #   방문 예정일 미래 필터가 이미 있어 완료된 옛 공사 확정은 자동 제외 (1건 수준).
        if status not in ('방문 예약', '공사 확정'):
            continue
