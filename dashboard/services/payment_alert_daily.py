...
def run_daily_scan_and_summary() -> Dict:
    ...
    def _check_sheet_note_mismatch(row: Dict, payments: List[Dict], u: int, v: int, w: int, notes_text: str = '', row_i: int = 0) -> Optional[Dict]:
        ...
        필터:
          - AA(수금완료) 체크된 프로젝트 skip — 매니저가 이미 완결 확인
          - 시트 음수 값 (반환 처리) skip
        ...
        """
        # 2026-07-11 AA 체크된 프로젝트 skip.
        #   R3611-YM 관측 — 매니저가 카드 수수료·이체 방식 등으로 실입금과 계약값이
        #   달라도 완결 처리 후 AA=True 체크. 시트 vs 메모 차이가 남아있어도 매니저는
        #   이미 인지·완결 상태. 재확인 알림은 노이즈.
        if row.get('aa', False):
            return None
