...
def parse_mail_body(body: str) -> Dict[str, Any]:
    ...
    if category == '홈페이지':
        ...
        # 시공 장소 — 폼 라벨 개정 공용 대응:
        #   구: "시공 원하시는 장소를 선택해주세요. - ★ ..." / 신(2026-09-21): "시공 장소"
        #   라벨 줄(시공…장소…) 다음 줄이 값, 종료는 기기 라벨("…기기 종류를 선택…").
        result['place'] = _clean(
            _safe_search(
                r'시공[^\n]*장소[^\n]*\n\s*([\s\S]*?)\s*\n[^\n]*기기 종류를 선택',
                body,
        ...
        # 필요한 기기 종류 — 폼 라벨 개정 공용 대응:
        #   구: "…기기 종류를 선택해 주세요. [중복선택가능] - ★ ..." / 신: "필요한 기기 종류를 선택해 주세요. [복수 선택 가능]"
        #   종료: 주소 라벨("(무료 )?방문 견적 받으실" / "주소를 입력해"). 복수선택 줄바꿈은 ', '로 정리.
        _device_raw = _safe_search(
            r'기기 종류를 선택[^\n]*\n\s*([\s\S]+?)\s*\n[^\n]*(?:방문 견적 받으실|주소를 입력해)',
            body,
        )
        result['device'] = _clean(re.sub(r'\s*\n\s*', ', ', _device_raw.strip()))
