...
def _register_visit_handlers(app):
    ...
    def handle_visit_message(event, client):
        ...
        # 0) 방문요청 인입 히든 채널 감지 — 워크플로가 보낸 메시지 파싱 후 처리
        # (bot_id 있어도 처리 — workflow 봇이 발송)
        intake_channel = os.getenv('SLACK_VISIT_INTAKE_CHANNEL_ID', '').strip()
        if intake_channel and channel == intake_channel and not thread_ts:
            text_full = event.get("text") or ''
            if text_full:
                threading.Thread(
                    target=_process_visit_intake_message,
...
# ─────────────────────────────────────────────────────────────
# 방문요청 인입 (히든 채널 라우팅, 2026-07-15)
# ─────────────────────────────────────────────────────────────
# 슬랙 워크플로 "거래처 (기타) 방문요청 등록" 이 시트 append 대신 히든 채널
# (SLACK_VISIT_INTAKE_CHANNEL_ID) 에 KEY: VALUE 양식 메시지 발송.
# 방문 일정 알림봇이 이 메시지를 감지해서 유형별 분기:
#   - 거래처/소개 → 시트 append (기존 sync 로직 재활용, 정상 lead_no 발번)
#   - 기타       → 시트 스킵 + ETC-xxxxxx pseudo-lead + Redis metadata
# 양쪽 다 #방문_일정 채널에 정식 카드 + Slack List 등록.
# 시트 잔재 zero, 매니저 진입점 그대로.
_INTAKE_FIELD_ALIASES = {
    '방문 유형': '플랫폼',
    '방문유형': '플랫폼',
    '유형': '플랫폼',
    '희망 서비스': '키워드',
...
def _parse_intake_message(text: str) -> dict:
    """워크플로가 보낸 KEY: VALUE 라인 텍스트를 dict 로 파싱.
    ...
    for line in (text or '').split('\n'):
        ...
        key = key.strip().lstrip('*').strip()  # 슬랙 볼드 마커 정리
        # 이모지·마크다운 제거
        key = re.sub(r'^[^\w가-힣]+', '', key).strip()
        value = value.strip().strip('*').strip()
        if not key or not value:
            continue
        canonical = _INTAKE_FIELD_ALIASES.get(key, key)
        # 첫 매치 우선 (같은 키가 두 번 오면 무시)
        if canonical not in result:
            result[canonical] = value
    return result


def _process_visit_intake_message(client, event) -> None:
    """히든 채널에서 워크플로 메시지 감지 시 유형별 분기 처리.

    - 기타 → ETC- pseudo-lead + Redis metadata + 카드/List (시트 스킵)
    - 거래처/소개 → 시트에 raw append (lead_no 빈 채) + sync 즉시 트리거
      (sync 가 lead_no 발번 + 카드/List 발송)
    """
    text = event.get("text") or ''
    channel_intake = event.get("channel", "")
    intake_ts = event.get("ts", "")

    try:
        data = _parse_intake_message(text)
    except Exception as exc:
        logger.error(f"[SLACK/방문인입] 파싱 실패: {exc} — text={text[:200]!r}",
                     exc_info=True)
        return

    platform = data.get('플랫폼', '').strip()
    name = data.get('고객명', '').strip()
    phone = data.get('고객 연락처', '').strip()
    address = data.get('방문 주소', '').strip()
    if not platform:
        logger.warning(f"[SLACK/방문인입] 방문 유형 미지정 — 파싱 결과: {data!r}")
        return
    if not (name or phone):
        logger.warning(f"[SLACK/방문인입] 고객명·연락처 둘 다 없음 — skip: {data!r}")
        return

    logger.info(
        f"[SLACK/방문인입] 감지: 플랫폼={platform} 이름={name!r} 연락처={phone!r} "
        f"주소={address[:30]!r}"
    )

    # 방문 예정일: '시작 ~ 종료' 범위 조립 (종료 있으면)
    vd_start = data.get('방문 예정일 시작', '').strip()
    vd_end = data.get('방문 예정일 종료', '').strip()
    if vd_start.startswith("'"):
        vd_start = vd_start[1:]
    if vd_end.startswith("'"):
        vd_end = vd_end[1:]
    if vd_end and vd_end != vd_start:
        visit_date_display = f"{vd_start} ~ {vd_end}"
    else:
        visit_date_display = vd_start

    inquiry = data.get('상담 내용', '').strip() or data.get('문의 내용', '').strip()
    keyword = data.get('키워드', '').strip()
    email = data.get('이메일', '').strip()
    user_name = (data.get('영업 담당자', '') or data.get('온라인 상담자', '')).strip().lstrip('@').strip()
    ...
    if platform == '기타':
        ...
        try:
            ...
            notice_channel, notice_ts = _post_visit_notice(
                ...
                user_id='', visit_date=visit_date_display,
            ...
            logger.info(
                f"[SLACK/방문인입/ETC] 처리 완료: {etc_lead_no} ({name}/{address[:30]})"
            )
        except Exception as exc:
            logger.error(
                f"[SLACK/방문인입/ETC] 처리 실패: {exc}", exc_info=True,
            )
        return

    # 거래처/소개 (or 그 외) — 시트 append (raw, lead_no 빈) + sync 트리거
    if platform not in ('거래처', '소개'):
        logger.warning(
            f"[SLACK/방문인입] 알 수 없는 플랫폼: {platform!r} — 그래도 시트 append"
        )
    try:
        ...
        if not cfg:
            logger.error("[SLACK/방문인입] ONLINE_LEADS_SHEET_ID 미설정 — append 실패")
            return
        ...
        row_data = {
            ...
            '플랫폼': platform,
            '상태': '방문 예약',
            '방문 예정일': (f"'{vd_start}" if vd_start else ''),
            '고객 연락처': phone,
            '이메일': email,
            '고객명': name,
            '방문 주소': address,
        ...
        new_row = [row_data.get(col, '') for col in LEAD_COLUMN_ORDER]

        manager = get_sheets_manager()
        manager.append_row(cfg['sheet_id'], cfg['sheet_name'], new_row)
        invalidate_leads_cache()
        logger.info(
            f"[SLACK/방문인입] 시트 append 완료: 플랫폼={platform} 이름={name}"
        )

        # sync 즉시 트리거 (lead_no 발번 + 카드 + List)
        from dashboard.services.lead_sync import sync_workflow_phone_leads
        sync_workflow_phone_leads()
    except Exception as exc:
        logger.error(
            f"[SLACK/방문인입] 시트 append or sync 실패: {exc}", exc_info=True,
        )
...
def _process_visit_thread_files(client, event) -> None:
    ...
    try:
        ...
        reply_text = (
            ...
            f">*추가 사진* : 카톡처럼 30장씩 나눠 올려도 같은 폴더에 이어서 저장됩니다"
        )
