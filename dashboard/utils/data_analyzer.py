...
def test_data_analyzer():
    ...
    try:
        ...
        logger.info("%s", regional.head())

        logger.info("\\n=== 미수금 분석 ===")
        outstanding = analyzer.get_outstanding_analysis()
        logger.info("총 미수금 건수: %d", outstanding['total_cases'])
        logger.info("총 미수금 금액: %s원", f"{outstanding['total_amount']:,}")

        logger.info("\\n=== 누락 데이터 분석 ===")
        missing = analyzer.check_missing_data()
        logger.info("전체 누락률: %.1f%%", missing['overall_missing_rate'])

    except Exception as e:
        logger.error("테스트 오류: %s", str(e))
